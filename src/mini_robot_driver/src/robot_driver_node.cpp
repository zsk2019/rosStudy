#include "mini_robot_driver/robot_driver_node.hpp"
...
RobotDriverNode::RobotDriverNode() : Node("robot_driver_node") {
  ...
  mode_service_ = create_service<mini_robot_interfaces::srv::SetRobotMode>(
      "/robot/set_mode",
      [this](
          const mini_robot_interfaces::srv::SetRobotMode::Request::SharedPtr
              request,
...
void RobotDriverNode::set_robot_mode(
...
RobotDriverNode::on_parameters_changed(
...
void RobotDriverNode::update_publish_timer() {
  if (timer_) {
    timer_->cancel();
  }
...
void RobotDriverNode::print_robot_status() {
...
int main(int argc, char* argv[]) {
  ...
  rclcpp::spin(std::make_shared<Mrb::RobotDriverNode>());
  rclcpp::shutdown();
  return 0;
}
